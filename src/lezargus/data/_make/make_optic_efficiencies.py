...
def make_irtf_primary_efficiency(basename: str) -> hint.LezargusSpectrum:
    """Load the IRTF primary mirror efficiency function (spectrum).

    Parameters
    ----------
    basename : str
        The basename of the internal data file of the IRTF primary efficiency.
        The paths are handled automatically.

    Returns
    -------
    efficiency_spectrum : LezargusSpectrum
        The IRTF primary mirror efficiency function spectrum.

    """
    # Parsing the filename.
    irtf_filename = functionality.parse_basename(basename=basename)
    mrt_table = astropy.table.Table.read(irtf_filename, format="ascii.mrt")
    # Extracting the needed data.
    wavelength = mrt_table["wavelength"]
    efficiency = mrt_table["primary"]
...
def make_irtf_secondary_efficiency(basename: str) -> hint.LezargusSpectrum:
    """Load the IRTF secondary mirror efficiency function (spectrum).

    Parameters
    ----------
    basename : str
        The basename of the internal data file of the IRTF secondary efficiency.
        The paths are handled automatically.

    Returns
    -------
    efficiency_spectrum : LezargusSpectrum
        The IRTF secondary mirror efficiency function spectrum.
    ...
    # Extracting the needed data.
    wavelength = mrt_table["wavelength"]
    efficiency = mrt_table["secondary"]
