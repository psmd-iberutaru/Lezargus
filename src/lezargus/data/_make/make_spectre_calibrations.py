...
def make_simulation_arclamp_spectrum(basename: str) -> hint.LezargusSpectrum:
    ...
    logging.error(
        ...
        message=(
            "Arclamp flux units are in ct s^-1 (or DN s^-1) and should be"
            " converted to W / m. properly."
        ),
    )
    wavelength_si = wavelength
    constant = 1e-11
    flux_si = flux * constant
    logging.warning(
        warning_type=logging.AccuracyWarning,
        message=(
            "Approximating the arc lamp level as about the flat lamp level,"
            f" multiplying by {constant}"
        ),
    )

    fake_arclamp_data_unit = lezargus.library.conversion.parse_astropy_unit(
        unit_input="W m^-1",
    )
