...
def cubic_interpolate_1d_function(x:hint.Array, y:hint.Array) -> hint.Callable[[hint.Array], hint.Array]:
    ...
    cubic_interpolate_function = scipy.interpolate.CubicSpline(x=x, y=y, bc_type="not-a-knot", extrapolate=True)
    ...
    def interpolate_1d_wrapper(input_data:hint.Array) -> hint.Array:
        ...
        if not (min(original_x) <= input_data & input_data <= max(original_x)).all():
            logging.warning(warning_type=logging.AccuracyWarning, message="Interpolating beyond original input domain, extrapolation is used.")
