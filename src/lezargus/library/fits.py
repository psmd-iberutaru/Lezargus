...
def read_fits_image_file(
    ...
    if not isinstance(data, np.ndarray):
        logging.warning(
            warning_type=logging.DataLossWarning,
            message=(
                "The FITS file {f} is expected to be an image-based FITS file,"
                " but its data is not that of an image. The return data is"
                " None.".format(f=filename)
            ),
        )
...
def read_fits_table_file(
    ...
    if not isinstance(data, astropy.table.Table | astropy.io.fits.FITS_rec):
        logging.warning(
            warning_type=logging.DataLossWarning,
            message=(
                "The FITS file {f} is expected to be a table-based FITS file,"
                " but its data is not that of an table. The returned table is"
                " None.".format(f=filename)
            ),
        )
        table = None
...
def write_fits_image_file(
    ...
    if not isinstance(header, dict | astropy.io.fits.Header):
        logging.error(
            ...
            message=(
                "The input header for the FITS file {f} is not an appropriate"
                " header, defaulting to an empty header.".format(f=filename)
            ),
        )
        logging.warning(
            warning_type=logging.DataLossWarning,
            message=(
                "The input header data, as follows, was ignored because it is"
                " not compatible: \n {hd}".format(hd=header)
            ),
        )
        header = None
    if not isinstance(data, np.ndarray):
        logging.error(
            error_type=logging.InputError,
            message=(
                "The input data for the FITS file {f} is not an appropriate"
                " data array. There is nothing to save, defaulting to a FITS"
                " file without any data.".format(f=filename)
            ),
        )
        logging.warning(
            warning_type=logging.DataLossWarning,
            message=(
                "The input data, as follows, was ignored because it is not"
                " compatible: \n {dt}".format(dt=data)
            ),
        )
        data = None
...
def write_fits_table_file(
    ...
    """Write a table to a FITS table file.
    ...
    if not isinstance(table, astropy.table.Table | astropy.io.fits.FITS_rec):
        logging.error(
            ...
            message=(
                "The input data for the FITS file {f} is not an appropriate"
                " table. There is nothing to save, defaulting to a FITS file"
                " without any table data.".format(f=filename)
            ),
        )
        logging.warning(
            warning_type=logging.DataLossWarning,
